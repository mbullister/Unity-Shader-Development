Shader "Teague/CompleteMap(UV2) CutoutMap(UV3) MaskMap(UV1or2)" {
	Properties {
		...
		_ClipTex("CutoutMap (UV3)", 2D) = "white" {}
	...
	SubShader {
		...
		Pass {   
			...
			fixed3 frag(vertexOutput i) : COLOR
			{
				...
				fixed clippingMask = tex2D(_ClipTex, i.uv2.xy * _ClipTex_ST.xy + _ClipTex_ST.z).y;
