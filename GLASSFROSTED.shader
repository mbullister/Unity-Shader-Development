Shader "Teague/Glass (Frosted)" {
	Properties{
		...
		[Toggle]_bThickRefraction("    Use Thick-Surface Refraction", Int) = 0
		...
		_DepthTex("ThicknessMap(UV2)", 2D) = "gray" {}
		_ThickBias("    Thickness Bias", Range(-1,3)) = 0
		...
		Pass{
			...
			uniform uint _bThickRefraction;
			...
			uniform half _ThickBias;
			uniform half _ThickScale;
	...
	{
		half Thickness = pow(tex2D(_DepthTex, i.uv1).y, 1);
		Thickness = Thickness / (pow(_ThickScale, 2) + 0.01); 
		Thickness = Thickness + _ThickBias;

		//_RefractionScale *= Thickness;
		...
		float ScreenDepth = saturate(distance(_WorldSpaceCameraPos, i.posWorld) / 3);
		...
		//ScreenCoords = ScreenCoords + (i.normalWorld * _R0 * _RefractionScale);
		
		//return float3(ScreenCoords.xy, 0);
		...
		float fresnel = Shlick(viewDirection, i.normalDir);

		_GlassColor = saturate(pow(_GlassColor, Thickness))* (1 - (fresnel * (1 - _FrostedStrength)));//  pow(1 - fresnel, 2);

		
		//float3 CubeNormal1 = normalize(lerp(i.normalWorld, normalDirection, ditherPattern1 + 0.125));
		float4 reflectionCube = texCUBElod(_Cube, float4(CubeCoords, _Roughness * 5));
		...
		float4 refractionCube = texCUBElod(_Cube, float4(refract(i.viewDir.xyz, i.normalWorld.xyz, 1 / _RefractionScale), 4 * _FrostedStrength));
		float3 refraction = pow((refractionCube.xyz * refractionCube.a * 6), 2.2);
		refraction *= _GlassColor;
		//return pow(refraction, 1/2.2); 
		//return refract(i.viewDir.xyz, i.normalWorld.xyz, 1.6);
		...
		if (_bThickRefraction == 1){
			totalLighting = lerp(screenRefraction, refraction, refractionCubeStrength);
