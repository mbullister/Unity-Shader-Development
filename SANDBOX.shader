	...
	SubShader {
		Pass {   
			...
			half3 frag(vertexOutput i) : COLOR
			{
				...
				localCoords = cross(i.viewDir, i.normalWorld);
				//return float3(dot(i.normalWorld.x, i.viewDir.x), dot(i.normalWorld.y, i.viewDir.y), dot(i.normalWorld.z, i.viewDir.z));
				localCoords = float3(dot(i.normalWorld.x, i.viewDir.x), dot(i.normalWorld.y, i.viewDir.y), dot(i.normalWorld.z, i.viewDir.z)) * _AnisoStrength;
				
				//return normalTest;
				
				//return localCoords;
				//localCoords = reflect(localCoords, i.viewDir);
				//return localCoords;
				...
				float3 normalDirectionTest2 = cross(i.viewDir, i.normalWorld);
				normalDirectionTest2 = normalize(cross(i.normalWorld, normalDirectionTest2));
				//return normalDirectionTest2;
				...
				float falloff = pow(1 - dot(reverseViewDirection, i.normalWorld), 3);
				
				ditherPattern1 = lerp(0, ditherPattern1, falloff * _AnisoStrength);
				//return ditherPattern1;
				ditherPattern2 = lerp(0, ditherPattern2, falloff * _AnisoStrength);
								
				float3 CubeNormal1 = normalize(lerp(i.normalWorld, normalDirectionTest2, ditherPattern1 * 0.25));
				//return CubeNormal1;
				float3 CubeSample1 = texCUBElod(_Cube, float4(reflect(viewDirection, CubeNormal1), _Roughness * 6));
				//return falloff;
				return pow(CubeSample1, 0.45454545);
				
				float3 CubeNormal2 = normalize(lerp(i.normalWorld, normalDirectionTest2, ditherPattern2 * 1.0));
				...
				float3 reflection = (CubeSample1 + CubeSample2) / 2;
